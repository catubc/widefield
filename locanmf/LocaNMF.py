...
def rank_linesearch(low_rank_video,
    ...
    for itr in range(maxiter_rank):
        ...
        lambda_iters,loc_save,save_lam,save_scale,save_per,save_spa,save_scratch = lambda_linesearch(low_rank_video,
                                         region_metadata,
                                         nmf_factors,
                                         nnt=nnt,
                                         verbose=verbose[1:],
        ...
        if not np.any(refit_flags):
            break
    return nmf_factors,loc_save,save_lam,save_scale,save_per,save_spa,save_scratch
...
def lambda_linesearch(video,
    ...
    for itr in range(maxiter_lambda):
        ...
        # Perform HALS
#         print(video.shape)
#         save_spatial_data=torch.clone(localized_factorization.spatial.data)
        hals_iters = hals(video,
                          ...
                          **kwargs)
#         if itr>0:
#             localized_factorization.spatial.data[np.where(sb==0)]=save_spatial_data[np.where(sb==0)]
        if verbose[0]:
            if device=='cuda': torch.cuda.synchronize()
        ...
        s1=localized_factorization.spatial.data.numpy().copy()
        save_spa.append(s1)
        s2=localized_factorization.spatial.scratch.numpy().copy()
        save_scratch.append(s2)
        sc=localized_factorization.scale.data.numpy().copy()
        save_per.append(sc)
        localized_factorization.scale.data.gt_(np.sqrt(100 / loc_thresh))
        sb=localized_factorization.scale.data.numpy().copy()
        save_scale.append(sb)
        if torch.sum(localized_factorization.scale.data) > 0:
            ...
            breakout = True
        sa=localized_factorization.lambdas.data.numpy().copy()
        save_lam.append(sa)
        if verbose[0]:
            if device=='cuda': torch.cuda.synchronize()
        ...
        if breakout:
            break
    return itr + 1, localized_factorization,np.array(save_lam),np.array(save_scale),np.array(save_per),np.array(save_spa),np.array(save_scratch)
